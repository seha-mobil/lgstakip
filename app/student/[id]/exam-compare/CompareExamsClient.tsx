...
export default function CompareExamsClient({ exams, studentColor }: { exams: any[], studentColor: string }) {
  ...
  return (
    <div>
      <div className="glass-card animate-fade-up" style={{ padding: '20px', marginBottom: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px' }}>
          <div>
            ...
            <select className="input" style={{ fontSize: '13px', padding: '10px' }} value={exam1Id} onChange={e => setExam1Id(e.target.value)}>
          ...
          <div>
            ...
            <select className="input" style={{ fontSize: '13px', padding: '10px' }} value={exam2Id} onChange={e => setExam2Id(e.target.value)}>
              {exams.map(e => <option key={e.id} value={e.id}>{e.trialExam.name} ({new Date(e.date).toLocaleDateString('tr-TR')})</option>)}
            </select>
          </div>
        </div>
      ...
      <div className="glass-card animate-fade-up" style={{ padding: '24px', animationDelay: '0.2s', marginBottom: '20px' }}>
        <div className="table-container">
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center', fontSize: '13px', minWidth: '400px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', color: 'var(--text3)' }}>
                <th style={{ padding: '12px', textAlign: 'left' }}>Ders</th>
                <th style={{ padding: '12px' }}>1. Net</th>
                <th style={{ padding: '12px' }}>2. Net</th>
              ...
              {SUBJECTS.map(sub => {
                ...
                return (
                  <tr key={sub.key} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '12px', fontFamily: 'var(--mono)' }}>{net1.toFixed(2)}</td>
                    <td style={{ padding: '12px', fontFamily: 'var(--mono)' }}>{net2.toFixed(2)}</td>
